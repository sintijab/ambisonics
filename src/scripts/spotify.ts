...
export async function handleSpotify(track: any) {
    try {
        ...
        // Track search
        const trackProvider = track.hub.providers.find((provider: { type: 'SPOTIFY' }) => provider.type === 'SPOTIFY');
        const isrc = `isrc%3A${track.isrc}`;
        const trackaActions = trackProvider.actions.find((action: { type: 'uri' }) => action.type === 'uri');
        const trackUri = trackaActions.uri.replace('spotify:search:', '');
        let trackInfo = await fetchSpotify(`api/search?${authQuery}&q=${trackUri}%2520${track.isrc ? isrc : ''}`);
        if (!trackInfo) {
            await fetchSpotify(`api/search?${authQuery}&q=${trackUri}%2520${track.isrc ? isrc : ''}`);
        }
        // Track details
        const { tracks: { items } } = trackInfo;
        const trackDetails = items.find((item: { name: string }) => track.title.includes(item.name));
        const trackId = trackDetails?.id;
        if (trackId) {
            let trackFeatures = await fetchSpotify(`api/features?${authQuery}&track_id=${trackId}`);
            if (!trackFeatures) {
                trackFeatures = await fetchSpotify(`api/features?${authQuery}&track_id=${trackId}`);
            }
            ...
            const clipContainer: HTMLElement = document.getElementById(`sound_clip-${recordings.length}`) as HTMLElement;
            ...
            let recommendations = await fetchSpotify(`api/recommendations?${authQuery}&seed_artists=${seed_artists}&seed_genres=${seed_genres}&seed_tracks=${seed_tracks}`);
            if (!recommendations) {
                recommendations = await fetchSpotify(`api/recommendations?${authQuery}&seed_artists=${seed_artists}&seed_genres=${seed_genres}&seed_tracks=${seed_tracks}`);
            }
